...
async function finalizeMatch(req, res, supabaseAdmin) {
  ...
  if (scoredPredictions.length > 0) {
    const { data: updatedPredictions, error: scoringError } = await supabaseAdmin
      .from('predictions')
      .upsert(scoredPredictions, { onConflict: 'id' })
      .select('id')

    if (scoringError) throw scoringError
    if ((updatedPredictions || []).length !== scoredPredictions.length) {
